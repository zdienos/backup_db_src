...
function checkLists {
    ...
    if ! [ -f $backuplistdb ] ; then
        echo "Missing backup Listdb. create $backuplistdb"
        exit
    fi;
}
...
function dumpSQL {
    ...
    if $WRITE_CHANGES && $BACKUP_MYSQL ; then
        ##/opt/lampp/bin/mysql -u $SQL_USER -p$SQL_PASSWD -Bse 'show databases' > $listfile
        printf "Dumping SQL Databases.. ";
        ##cat $listfile | while read line
        cat $backuplistdb | while read line
        do
            dbname=$line
            if [ $line != "information_schema" ] ;
            then
                ##/opt/lampp/bin/mysqldump --events --ignore-table=mysql.events -u $SQL_USER -p$SQL_PASSWD $dbname > $tempdir/$dbname.sql
		/opt/lampp/bin/mysqldump --skip-lock-tables --ignore-table=mysql.events -u $SQL_USER -p$SQL_PASSWD $dbname > $tempdir/$dbname.sql 
            fi
        done
        printf "Ok\n"
    else printf "Skipping\n"
